...
class GitStats:
    ...
    def __init__(self, data: dict):
        ...
        self.coupling: dict[str, list[dict]] = defaultdict(
            list
        )  # path → [{peer, co_changes, score}]
    ...
    def file_coupling(self, path: str) -> list[dict]:
        return sorted(
            self.coupling.get(path, []), key=lambda x: x["score"], reverse=True
        )
    ...
    def format_risk_block(self, path: str) -> list[str]:
        ...
        if risk:
            ...
            lines.append(
                f"Git risk: {icon} {risk['risk']} ({risk['changes']} changes in {self.period_days} days)"
            )
        ...
        if coupling:
            ...
            for c in coupling[:5]:
                lines.append(
                    f"  - {c['peer']} (coupling: {c['score']:.2f}, {c['co_changes']} co-changes)"
                )
        return lines
...
class ASTGraph:
    ...
    def _module_aliases(self, module_id: str, path: str) -> set[str]:
        ...
        for marker in self.SOURCE_ROOT_MARKERS:
            if tuple(normalized_parts[: len(marker)]) == marker and len(
                normalized_parts
            ) > len(marker):
    ...
    def query_file(self, file_query: str) -> str:
        ...
        # 类和函数
        classes = [
            n for n in self.contains_children.get(mid, []) if n["type"] == "Class"
        ]
        top_funcs = [
            n for n in self.contains_children.get(mid, []) if n["type"] == "Function"
        ]
        ...
        if classes:
            ...
            for cls in classes:
                ...
                methods = [
                    n
                    for n in self.contains_children.get(cls["id"], [])
                    if n["type"] == "Function"
                ]
    ...
    def query_hub_analysis(self, top_n: int = 10) -> str:
        ...
        fan_in = {
            target: len(sources)
            for target, sources in self.internal_imports_reverse.items()
        }
        fan_out = {
            source: len(targets)
            for source, targets in self.internal_imports_forward.items()
        }
        ...
        if top_fan_out:
            for i, (mid, count) in enumerate(top_fan_out, 1):
                path = self.resolve_to_path(mid) or ""
                out.append(
                    f"  {i}. {mid} — imports {count} internal module(s)  [{path}]"
                )
        else:
            out.append("  (no internal import relationships found)")
    ...
    def query_summary(self) -> str:
        ...
        for dk in sorted(dir_stats.keys()):
            ...
            if s["class_names"]:
                ...
                suffix = (
                    f" ... +{len(s['class_names']) - 8}"
                    if len(s["class_names"]) > 8
                    else ""
                )
...
""",
    ...
    parser.add_argument(
        "--file", dest="file_query", help="Show structure and imports of a file"
    )
    parser.add_argument(
        "--who-imports",
        dest="who_imports",
        help="Find modules that import the given module",
    )
    parser.add_argument(
        "--impact", dest="impact_query", help="Show impact radius (deps + dependents)"
    )
    parser.add_argument(
        "--hub-analysis", action="store_true", help="Show top fan-in/fan-out modules"
    )
    parser.add_argument(
        "--summary", action="store_true", help="Show per-directory structural summary"
    )
    parser.add_argument(
        "--top",
        type=int,
        default=10,
        help="Number of results for hub-analysis (default: 10)",
    )
    parser.add_argument(
        "--git-stats",
        dest="git_stats_path",
        metavar="GIT_STATS_JSON",
    ...
    if args.git_stats_path:
        ...
        if not gs_path.exists():
            sys.stderr.write(
                f"[WARNING] git_stats file not found: {gs_path}, ignoring\n"
            )
        else:
            try:
