...
def _should_skip_path(repo_path: Path, path: Path) -> bool:
    ...
    if path.is_file() and any(
        path.name.endswith(suffix) for suffix in EXCLUDE_FILE_SUFFIXES
    ):
        return True
    return False
...
def _apply_cli_customizations(
    ...
    if cli_extensions:
        for item in cli_extensions:
            if "=" not in item:
                warnings.append(
                    f"ignored invalid extension mapping {item!r}, expected EXT=LANG"
                )
                continue
            ...
            try:
                ...
                if not lang:
                    warnings.append(
                        f"ignored empty language name for extension {ext_part!r}"
                    )
                    continue
    ...
    if cli_queries:
        for query_item in cli_queries:
            if len(query_item) != 3:
                warnings.append(
                    f"ignored malformed query: expected 3 parts, got {len(query_item)}"
                )
                continue
            ...
            if query_type not in ("struct", "imports"):
                warnings.append(
                    f"ignored unknown query type {query_type!r} for language {lang!r}"
                )
                continue
...
def _load_language_customizations(
    ...
    if explicit_config_path:
        ...
        resolved_path = (
            config_path if config_path.is_absolute() else (repo_path / config_path)
        )
        ...
        if not isinstance(config_data, dict):
            warnings.append(
                f"language config ignored because root value is not an object: {resolved_path}"
            )
            return (
                extension_map,
                lang_queries,
        ...
        if isinstance(extensions, dict):
            for raw_ext, raw_lang in extensions.items():
                if (
                    isinstance(raw_ext, str)
                    and isinstance(raw_lang, str)
                    and raw_lang.strip()
                ):
                    try:
        ...
        if isinstance(queries, dict):
            for raw_lang, raw_query_parts in queries.items():
                if (
                    isinstance(raw_lang, str)
                    and raw_lang.strip()
                    and isinstance(raw_query_parts, dict)
                ):
                    lang = raw_lang.strip().lower()
...
def _load_languages(
    ...
    targets = (
        requested
        if requested
        else sorted(set(extension_map.values()) | set(lang_queries.keys()))
    )
    ...
    if not languages:
        sys.stderr.write(
            "[ERROR] 没有可用的语言解析器，请安装 tree-sitter-language-pack\n"
        )
        sys.exit(1)
    return languages
...
def extract_file(
    ...
    if struct_q_text.strip():
        try:
            ...
            for pattern_idx, captures in QueryCursor(struct_query).matches(
                tree.root_node
            ):
                ...
                if is_class:
                    ...
                    class_ranges.append(
                        (def_node.start_byte, def_node.end_byte, node_id)
                    )
                    edges.append(
                        {"source": module_id, "target": node_id, "type": "contains"}
                    )
                else:
                    parent_id = module_id
                    for cls_start, cls_end, cls_id in class_ranges:
                        if (
                            cls_start <= def_node.start_byte
                            and def_node.end_byte <= cls_end
                        ):
                            parent_id = cls_id
                            break
    ...
    if import_q_text.strip():
        try:
            ...
            for _pattern_idx, captures in QueryCursor(import_query).matches(
                tree.root_node
            ):
                for mod_node in captures.get("mod", []):
                    target = (
                        source[mod_node.start_byte : mod_node.end_byte]
                        .decode("utf-8", "replace")
                        .strip("\"'<> ")
                    )
                    if target:
                        edges.append(
                            {"source": module_id, "target": target, "type": "imports"}
                        )
        except Exception as e:
...
def apply_max_nodes(
    ...
    kept_edges = [e for e in edges if e["source"] in kept_ids or e["type"] == "imports"]
...
def main() -> None:
    ...
    if not (repo_path / ".git").exists():
        sys.stderr.write(
            f"[WARNING] .git not found in {repo_path}, may not be a git repo\n"
        )
    ...
    # 处理 CLI 自定义参数
    cli_ext_override, cli_query_override, cli_warnings, cli_custom_query_languages = (
    ...
    languages_with_structural_queries = sorted(
        lang
        for lang, query_parts in lang_queries.items()
    ...
    for file_path, lang_name in source_files:
        ...
        if lang_name not in languages_with_structural_queries:
            module_only_file_counts[lang_name] = (
                module_only_file_counts.get(lang_name, 0) + 1
            )
        if nodes:
